...
{#if $user_identifier !== ""}
    <div class="dropdown dropdown-bottom dropdown-end">
        ...
        <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-300 rounded-box w-52">
            <form method="post" action={`/logout?redirectTo=${$page.url.pathname}`}>
    ...
    <div class="modal-box">
        ...
        <form method="post" action={`/login?redirectTo=${$page.url.pathname}`}>
            <label class={`input input-bordered flex items-center gap-2 mt-2 ${failed ? "input-error" : ""}`}>
                ...
                <input type="text" name="email" class="grow border-none focus:ring-0" placeholder="Email" />
            </label>
            <label class={`input input-bordered flex items-center gap-2 mt-2 ${failed ? "input-error" : ""}`}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 opacity-70"><path fill-rule="evenodd" d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z" clip-rule="evenodd" /></svg>
                <input type="password" name="password" class="grow border-none focus:ring-0" placeholder="Password" />
            </label>
